...
function! GetTagName(curline)
	...
	let leftlineS = GetLine(left)
	let leftlineE = GetLine(left+1)
	let max = line("$")
	if leftlineE > max
	    let leftlineE = max
	endif
	" let ret =  matchstr(b:tags, "[^\n]*", index) . " [" . leftlineS . " ~ " . leftlineE . "] " . getline(leftlineS)
	let ret =  matchstr(b:tags, "[^\n]*", index) . " [" . leftlineS . " ~ " . leftlineE . "] "
    endif
...
function! s:SetTagDisplay()
	...
	if &ruler
	    ...
	    " customized a bit
            let &statusline='%3*%f%m%r%h%w %0* %{TagName()} %=%3* [%{&ff},%Y] [%4v, %0*%4l%3*/%L] %P'
	else
